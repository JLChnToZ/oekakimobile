...
public class ColorPaletteSelector extends View {
	...
	private int getIndexByPosition(float x, float y) {
		...
		if(x > widthFixed) return -1;
		return index;
	}
	
	private class EventHandler  implements OnLongClickListener, OnTouchListener, OnClickListener {
		@Override
		public void onClick(View v) {
			if(v != ColorPaletteSelector.this) return;
		...
		public boolean onLongClick(View v) {
			if(v != ColorPaletteSelector.this) return false;
			int index = getIndexByPosition(posX, posY);
